...
use self::test_utilities::utilities::{
    engine_run_test, generate_id, get_pods, get_pvc, init, is_pod_restarted_env, log_manager, FuncTestsSecrets,
};
...
fn digitalocean_doks_deploy_a_working_environment_with_no_router() {
    ...
    engine_run_test(|| {
        ...
        match environment_for_delete.delete_environment(
            &log_manager,
            Kind::Do,
            &context_for_delete,
            &env_action_for_delete,
        ) {
            TransactionResult::Ok => assert!(true),
            TransactionResult::Rollback(_) => assert!(false),
            TransactionResult::UnrecoverableError(_, _) => assert!(false),
...
fn digitalocean_doks_deploy_a_working_environment_and_pause() {
    ...
    engine_run_test(|| {
        ...
        match environment.pause_environment(&log_manager, Kind::Do, &context_for_delete, &env_action) {
            TransactionResult::Ok => assert!(true),
            TransactionResult::Rollback(_) => assert!(false),
            TransactionResult::UnrecoverableError(_, _) => assert!(false),
        };
        ...
        match environment.deploy_environment(&log_manager, Kind::Do, &ctx_resume, &env_action) {
            TransactionResult::Ok => assert!(true),
            TransactionResult::Rollback(_) => assert!(false),
            TransactionResult::UnrecoverableError(_, _) => assert!(false),
        };
        ...
        // Cleanup
        match environment.delete_environment(&log_manager, Kind::Do, &context_for_delete, &env_action) {
            TransactionResult::Ok => assert!(true),
            TransactionResult::Rollback(_) => assert!(false),
            TransactionResult::UnrecoverableError(_, _) => assert!(false),
        };
...
fn digitalocean_doks_deploy_a_working_environment_with_domain() {
    ...
    engine_run_test(|| {
        ...
        match environment_delete.delete_environment(&log_manager, Kind::Do, &context_for_delete, &env_action_for_delete)
        {
            TransactionResult::Ok => assert!(true),
            TransactionResult::Rollback(_) => assert!(false),
            TransactionResult::UnrecoverableError(_, _) => assert!(false),
...
fn digitalocean_doks_redeploy_same_app() {
    ...
    engine_run_test(|| {
        ...
        match environment_redeploy.deploy_environment(&log_manager, Kind::Do, &context_bis, &env_action_redeploy) {
            TransactionResult::Ok => assert!(true),
            TransactionResult::Rollback(_) => assert!(false),
            TransactionResult::UnrecoverableError(_, _) => assert!(false),
        };
...
fn digitalocean_doks_deploy_a_not_working_environment_and_then_working_environment() {
    ...
    engine_run_test(|| {
        ...
        match environment_for_delete.delete_environment(&log_manager, Kind::Do, &context_for_delete, &env_action_delete)
        {
            TransactionResult::Ok => assert!(true),
            TransactionResult::Rollback(_) => assert!(false),
            TransactionResult::UnrecoverableError(_, _) => assert!(false),
...
fn digitalocean_doks_deploy_ok_fail_fail_ok_environment() {
    ...
    engine_run_test(|| {
        ...
        match delete_env.delete_environment(&log_manager, Kind::Do, &context_for_delete, &env_action_delete) {
            TransactionResult::Ok => assert!(true),
            TransactionResult::Rollback(_) => assert!(false),
            TransactionResult::UnrecoverableError(_, _) => assert!(false),
        };
...
fn digitalocean_doks_deploy_a_non_working_environment_with_a_working_failover() {
    ...
    engine_run_test(|| {
        ...
        match delete_env.delete_environment(&log_manager, Kind::Do, &context_deletion, &env_action_delete) {
            TransactionResult::Ok => assert!(true),
            TransactionResult::Rollback(_) => assert!(false),
            TransactionResult::UnrecoverableError(_, _) => assert!(false),
        };
...
fn digitalocean_doks_deploy_a_non_working_environment_with_a_non_working_failover() {
    ...
    engine_run_test(|| {
        ...
        match delete_env.delete_environment(&log_manager, Kind::Do, &context_for_deletion, &env_action_delete) {
            TransactionResult::Ok => assert!(true),
            TransactionResult::Rollback(_) => assert!(false),
            TransactionResult::UnrecoverableError(_, _) => assert!(true),
        };
