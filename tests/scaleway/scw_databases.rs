...
fn deploy_an_environment_with_db_and_pause_it() {
    ...
    engine_run_test(|| {
        ...
        match environment.pause_environment(&log_manager, Kind::Scw, &context, &env_action) {
            TransactionResult::Ok => assert!(true),
            TransactionResult::Rollback(_) => assert!(false),
            TransactionResult::UnrecoverableError(_, _) => assert!(false),
        };
...
fn postgresql_failover_dev_environment_with_all_options() {
    ...
    engine_run_test(|| {
        ...
        match environment_check.deploy_environment(&log_manager, Kind::Scw, &context, &env_action_fail_ok) {
...
fn postgresql_deploy_a_working_environment_and_redeploy() {
    ...
    engine_run_test(|| {
        ...
        match environment_delete.delete_environment(&log_manager, Kind::Scw, &context_for_delete, &env_action_delete) {
            TransactionResult::Ok => assert!(true),
            TransactionResult::Rollback(_) => assert!(false),
